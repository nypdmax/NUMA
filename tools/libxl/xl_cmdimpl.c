							  ...
							  int node_index)
{
    ...
    if (!xlu_cfg_get_list (config, "cpus", &cpus, &num_cpus, 1) ||
			!xlu_cfg_get_string (config, "cpus", &buf, 0)){
		// Validate if cpus injection is successful. [ck]
		fprintf(stderr, "[ck] Validating: cpus string is %s\n", buf);
                                        ...
                                        int *recv_fd)
{
    ...
    close(sendpipe[0]); // child uses sendpipe to send msg
    close(recvpipe[1]); // child uses recvpipe to receive msg
    *send_fd = sendpipe[1];
    *recv_fd = recvpipe[0];
...
/*
 * This is for numa migration between host's nodes. [ck]
 * We can use libxl_domain_info(libxl_ctx *ctx, libxl_dominfo *info_r,uint32_t domid)
 * to retrive a domain's info, xl domain info is much similar to xc_domain_info 
 *
                           ...
                           const char *override_config_file)
{
    ...
    if (rc_buf) {
		...
		/* Q: How can VCPUs be pinned to a node before libxl_domain_resume()? [ck]
		 * Maybe I have to alter the configuration file?
		 */
        rc = libxl_domain_resume(ctx, domid, 1, 0);
        if (!rc) fprintf(stderr, "migration sender: Resumed OK.\n");
...
/* 
 * Looks like this is the migration function. [ck]
*/
int main_migrate(int argc, char **argv)
{
	...
	2.has_arg is: no_argument (or 0) if the option does not take an argument; 
	required_argument (or 1) if the option requires an argument; 
	or optional_argument (or 2) if the option takes an optional argument.
	
	3.flag specifies how results are returned for a long option. 
	...
	Otherwise, getopt_long() returns 0, and flag points to a variable which is set to val if the option is found, 
	but left unchanged if the option is not found.
	
	4.val is the value to return, or to load into the variable pointed to by flag.
	*/
	
	/* Added by ck.[ck]
	SWITCH_FOREACH_OPT(opt, "FC:s:e:n", opts, "migrate", 2) {
	*/
    SWITCH_FOREACH_OPT(opt, "FC:s:e:n", opts, "migrate", 2) {
    case 'C':
